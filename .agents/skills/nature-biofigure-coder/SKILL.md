...
- When the user wants to reuse FigureYa modules, or when a local FigureYa checkout is available, treat FigureYa as an R template backend. Match recipes with `scripts/figureya_module_audit.py match <recipes_dir> --repo <FigureYa_repo> --out <matches.tsv> --markdown <matches.md>`, then inspect the selected module's Rmd, easy_input files, example image, and `top_confidence` before adapting code.
- When adapting FigureYa modules for Nature-style output, keep the module source read-only. Accept the FigureYa checkout as an explicit path such as `<FigureYa_repo>` or a user-provided environment variable, then source `<FigureYa_repo>/styles/nature_figure_style.R` when present. Check `<FigureYa_repo>/docs/nature_style_audit.tsv` or regenerate it with `python <FigureYa_repo>/scripts/figureya_nature_style_audit.py --repo <FigureYa_repo> --inventory <FigureYa_repo>/docs/module_inventory.tsv --out <FigureYa_repo>/docs/nature_style_audit.tsv --report <FigureYa_repo>/docs/nature_style_summary.md`.
