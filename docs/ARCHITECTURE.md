...
This repository is a Codex skill suite for biomedical figure evidence, reproducible plotting, figure compliance, manuscript-facing academic writing, domain result interpretation, and academic deliverables. The v0.5 architecture keeps the skills independent, but defines shared evidence handoffs so figures, prose, reviews, PPT plans, and teaching materials can be checked without silently inventing claims.
